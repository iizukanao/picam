...
HTTPLiveStreaming *_hls_create(int num_recent_files, int is_audio_only, MpegTSCodecSettings *settings) {
  ...
  if (is_audio_only) {
    mpegts_ctx = mpegts_create_context_audio_only(settings);
  } else {
    mpegts_ctx = mpegts_create_context(settings);
  }
  AVFormatContext *format_ctx = mpegts_ctx.format_context;
  hls->audio_ctx = mpegts_ctx.codec_context_audio;
  hls->video_ctx = mpegts_ctx.codec_context_video;
